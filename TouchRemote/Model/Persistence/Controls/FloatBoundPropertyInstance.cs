...
namespace TouchRemote.Model.Persistence.Controls
{
    ...
    public class FloatBoundPropertyInstance : ImplInstance, INotifyPropertyChanged
    {
        ...
        public IImplDescriptor Descriptor
        {
            ...
            set
            {
                PropertyChanged.ChangeAndNotify(ref _Descriptor, value, () => Descriptor, (oldValue, newValue) => Impl = newValue?.Manager.GetFloatBoundPropertyInstance(newValue));
            }
        }

        private FloatBoundProperty _Impl;
        [XmlIgnore]
        public FloatBoundProperty Impl
        {
            get
            {
                return _Impl;
        ...
        internal override InflateResult Inflate(PluginManager pluginManager)
        {
            ...
            {
                _Descriptor = pluginManager.GetFloatBoundPropertyDescriptor(Type);
                if (_Descriptor != null)
                {
                    _Impl = pluginManager.GetFloatBoundPropertyInstance(_Descriptor);
                    if (_Impl != null)
                    {
                    ...
                    {
                        return InflateResult.WithError(string.Format("Failed to find FloatBoundProperty instance for type \"{0}\"", _Descriptor.Type.FullName));
                    }
                }
            }
